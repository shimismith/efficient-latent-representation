...
def toVAE():
    BATCH_SIZE = 50
    _, test_loader = setup_data_loaders(batch_size=BATCH_SIZE, normalize=True)
    mono_depth = ImageFolder(root='result_bts_nyu_v2_pytorch_densenet161/raw/', transform=transforms.Compose([transforms.ToTensor(), transforms.Resize(128), transforms.Normalize(0.5)]))
    device = torch.device("cuda:0" if torch.cuda.is_available() else 'cpu')

    for i in range(0, 100, 10):
        gt_rgb = test_loader.dataset[i][:3].permute(1, 2, 0)*0.5 + 0.5
        gt_depth = test_loader.dataset[i][3]
        input = torch.cat((test_loader.dataset[i][:3], mono_depth[i])).unsqueeze(0).to(device)
        checkpoint = torch.load('./models/rgbd_pvae/', map_location=torch.device('cpu'))
        vae = torch.nn.Module.load_state_dict(checkpoint['rgbd_pvae.save'])
        reconstructed = vae.reconstruct(input).cpu().detach()[0]
        reconstruct_rgb = reconstructed[:3].permute(1, 2, 0)*0.5 + 0.5
        reconstruct_depth = reconstructed[3]

        save_image(gt_rgb, 'results/gt_rgb{}'.format(i))
        save_image(gt_depth, 'results/gt_depth{}'.format(i))
        save_image(reconstruct_rgb, 'results/reconstruct_rgb{}'.format(i))
        save_image(reconstruct_depth, 'results/reconstruct_depth{}'.format(i))
